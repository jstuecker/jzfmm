    ...
    for name in ('src', 'packaging'):
        for path in (root / name).rglob('*'):
            ...
            if path.is_file() and path.suffix not in {'.so', '.whl', '.pyc'}:
                if relative.as_posix() != 'src/jzfmm/_version.py':
                    paths.add(path)
...
def wheel_manifest(directory):
    ...
    with zipfile.ZipFile(wheels[0]) as archive:
        ...
        licenses = [n for n in archive.namelist() if n.endswith('.dist-info/licenses/LICENSE')]
        if len(licenses) != 1 or not archive.read(licenses[0]).strip():
            raise RuntimeError(f'Missing project license in {wheels[0]}')
...
def dependencies(mode, jax_version):
    common = ['build', 'setuptools>=77', 'wheel', 'auditwheel', 'scikit-build-core==0.12.2',
              'nanobind==2.9.2', 'cmake==4.3.1', 'numpy>=2.2.6']
    if mode == 'main':
        return ['build', 'setuptools>=77', 'wheel']
    cuda = mode[2:]
...
def stage_python_package(root, state):
    ...
    package = staged / 'packaging/jzfmm'
    package.mkdir(parents=True, exist_ok=True)
    for name in ('pyproject.toml', 'README.md'):
        shutil.copy2(root / 'packaging/jzfmm' / name, package / name)
...
def build_one(root, mode, py_version, source, output, image, jax_version):
    package = 'jzfmm' if mode == 'main' else f'jzfmm-{mode}'
