...
RESULTS_FILE = LOG_DIR / "loss_experiment.csv"
...
def load_cached_result(N: int, mode: str, ntc: float, ltfrac: float, softening: float, disp: float):
    ...
    with RESULTS_FILE.open(newline="") as f:
        for row in csv.DictReader(f):
            if _row_key(row) == key:
                print(f"Loading cached loss experiment from {RESULTS_FILE}: {key}")
...
def loss_experiment(
    ...
    cached = load_cached_result(N, mode, ntc, ltfrac, softening, disp)
    ...
    part_ref = fmdj.data.Particles(pos=pos, vel=vel, mass=mass)
    ...
    save_result(N, mode, ntc, ltfrac, softening, disp, target_dloss, actual_dloss)
    return target_dloss, actual_dloss


def plot_loss_experiment(N: int = int(1e4), ltfrac: float = 1e-2, softening: float = 0.1, disp: float = 0.5):
    fig, ax = plt.subplots(1, 1, figsize=(6, 4))

    for mode in ("pos", "mass"):
        times = ts[1:] if mode == "mass" else ts
        res = [loss_experiment(N=N, mode=mode, ntc=t, ltfrac=ltfrac, softening=softening, disp=disp) for t in times]
    ...
    LOG_DIR.mkdir(parents=True, exist_ok=True)
    path = LOG_DIR / f"loss_experiment_N{N:g}_ltfrac{_key_value(ltfrac)}_soft{_key_value(softening)}_disp{_key_value(disp)}.pdf"
