...
print(f"============== Lowest level Ilist build (1024*1024), newsort ==============")
...
il, r2il, ispl = cj.knn.build_ilist_recursive.jit(
    xleaf2, llvl2, nleaf2, max_size=32*64, refine_fac=8, num_part=len(posz), k=16, boxsize=boxsize)

par = (xleaf, llvl, nleaf, spl2, il, ispl)

radii, il, r2il, ispl = cj.knn.build_ilist_knn.jit(*par, alloc_fac=256, k=k, sort=True, boxsize=boxsize)
ispl.block_until_ready()

radii2, il2, ri2l, ispl2 = cj.knn.build_ilist_knn.jit(*par, alloc_fac=256, k=k, sort=True, boxsize=boxsize)

rnn, inn = cj.knn.ilist_knn_search.jit(posz, spl, xleaf, llvl, il, ri2l, ispl, k=k, boxsize=boxsize)
...
for rfac in 2, 4, 8, 16:
    ...
    timer.timeit_jit(cj.knn.build_ilist_knn.jit, *par, alloc_fac=222, k=k, sort=True, name="sort")

    radii,il,r2il,ispl = cj.knn.build_ilist_knn.jit(*par, alloc_fac=222, k=k, sort=False, boxsize=boxsize)
    if read:
        radiiref = jnp.load(f"logs/tmp/ilist_knn_radii_{N}_{rfac}.npy")
        ...
        print(jnp.allclose(il[:ispl[-1]], ilref[:ispl[-1]]), 
              ...
              jnp.allclose(radii[:ispl[-1]], radiiref[:ispl[-1]], rtol=1e-3))
        # print(radii[0:6], radiiref[0:6])
        # print(ispl[0:7], isplref[0:7])

    else:
        jnp.save(f"logs/tmp/ilist_knn_radii_{N}_{rfac}.npy", np.array(radii))
