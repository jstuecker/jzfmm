...
def time_interactions(timer, N, p, softening=1e-3):
    ...
    for tag in "base", "cuda":
        cfg = fmdj.Config(tags=(tag, "base"), p=p, softening=softening)

        Loc1 = timer.timeit_jit(fmdj.multipoles.ilist_node_to_node.jit, octree.xnode, octree.mp, 
            ilist, iranges[0:2], cfg=cfg, name = "node2node_" + tag)
        Loc2 = timer.timeit_jit(fmdj.multipoles.ilist_leaf_to_node.jit, octree.xnode, posz, massz, 
            octree.leaf_particle_bounds, ilist, iranges[1:3], cfg=cfg, name="leaf2node_" + tag)
        phi2 = timer.timeit_jit(fmdj.multipoles.ilist_leaf_to_leaf.jit, posz, massz, 
            octree.leaf_particle_bounds, ilist, iranges[3:5], cfg=cfg, name="leaf2leaf_" + tag)
        
        phi4 = timer.timeit_jit(fmdj.fmm.evaluate_interaction_lists.jit,
            octree, posz, massz, ilist, nilist, sort=False, cfg=cfg, name="total_" + tag)

    # Base only functions 
    cfg = fmdj.Config(tags=(tag, "base"), p=p, softening=softening)
    phi1 = timer.timeit_jit(fmdj.multipoles.ilist_node_to_leaf.jit, octree.xnode, octree.mp, 
        posz, octree.leaf_particle_bounds, ilist, iranges[2:4], cfg=cfg, name="node2leaf_base")
...
for N in int(1e4), int(1e5), int(3e5), int(1e6), int(2e6):
    ...
    time_interactions(timer, N, p=4, softening=1e-2)
...
for p in (1,2,3,4,5):
    ...
    time_interactions(timer, N=int(2e6), p=p, softening=1e-2)
