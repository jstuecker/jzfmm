...
class WheelBuilderTests(unittest.TestCase):
    def setUp(self):
        ...
        project = self.root / 'packaging/jzfmm'
        project.mkdir()
        for name in ('pyproject.toml', 'README.md'):
            (project / name).write_text('fixture')
        for name in ('jzfmm', 'jzfmm_utils'):
    ...
    def fake_run(self, *args, **kwargs):
        ...
        if args[:2] == ['uv', 'build']:
            ...
            with zipfile.ZipFile(out / 'jzfmm_cu13-1.0.0-cp312-cp312-linux_x86_64.whl', 'w') as z:
                z.writestr('fixture', 'not a real binary')
                z.writestr('fixture.dist-info/licenses/LICENSE', 'MIT license fixture')
    ...
    def test_missing_license_is_rejected(self):
        ...
        output.mkdir()
        with zipfile.ZipFile(output / 'fixture.whl', 'w') as archive:
            archive.writestr('fixture', 'no license')
        with self.assertRaisesRegex(RuntimeError, 'Missing project license'):
            builder.wheel_manifest(output)
    ...
    def test_main_does_not_compile_or_repair(self):
        ...
        self.assertNotEqual(Path(build[-1]), self.root / 'packaging/jzfmm')

    def test_setuptools_staging_excludes_old_build_artifacts(self):
        old = self.root / 'packaging/jzfmm/build/lib'
        ...
        self.assertTrue((package / '../../src/jzfmm/__init__.py').is_file())
