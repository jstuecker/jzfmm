...
static constexpr int OPENING_BY_SOFTENED_ANGLE = 1;
...
template<>
struct OpeningCriterion<OPENING_BY_SOFTENED_ANGLE> {
    template<typename tvec>
    struct Params {
        tvec theta;
        tvec softening;
    ...
    __device__ __forceinline__ static bool should_open(
        ...
        // Include epsilon in the scale to keep its square finite as well.
        if(params.softening != tvec(0))
            scale_exp = max(scale_exp, normal_ilogb(abs(params.softening)));
        const tvec eps_scaled = mulpow2(params.softening, -scale_exp);
        const Vec<dim,tvec> dx_scaled = mulpow2(dx, -scale_exp);
        const Vec<dim,tvec> extent_scaled =
            nonpositive_exp2<tvec>(levelsA - scale_exp)
            + nonpositive_exp2<tvec>(levelsB - scale_exp);
        ...
        const bool unbounded = unbounded_node<dim,tvec>(nodeA.level)
            | unbounded_node<dim,tvec>(nodeB.level);
        return unbounded | (tvec(0.25) * extent_scaled.norm2()
            >= params.theta * params.theta * (dx_scaled.norm2() + eps_scaled * eps_scaled));
    }
};
